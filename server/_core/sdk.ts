...
class SDKServer {
  ...
  async authenticateRequest(req: Request): Promise<User> {
    ...
    // Special handling for Management UI Preview Panel
    // When cookies are blocked by browser (third-party cookie policy),
    // fall back to owner user if request is from manus.im or has from_webdev=1
    if (!sessionCookie) {
      const origin = req.headers.origin || req.headers.referer || '';
      const url = req.url || '';
      const isManagementUI = origin.includes('manus.im') || 
                            origin.includes('manus-asia.computer') || 
                            url.includes('from_webdev=1') ||
                            req.headers['x-manus-preview'] === '1';
      logger.info('[Auth] No session cookie. Origin:', origin, 'URL:', url, 'isManagementUI:', isManagementUI);
      if (isManagementUI) {
        logger.info('[Auth] Management UI detected, using owner user');
